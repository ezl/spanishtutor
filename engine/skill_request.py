...
SKILL_LOOKUP_PROMPT = """A Spanish student asked for a lesson. Map their request to skills from this curriculum.

Curriculum:
{listing}

Student's request: "{phrase}"

Reply with ONLY the matching skill ids, comma-separated, most likely first. Reply with exactly NONE if nothing in the curriculum matches. Never invent an id — every id you return must appear verbatim in the list above."""
...
async def _model_candidates(phrase: str) -> list[dict]:
